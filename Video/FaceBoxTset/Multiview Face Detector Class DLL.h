...
class CFaceDetector;	//前向声明，这是避免包含FaceDetector.h的关键。
...
{
public:
	virtual int DDD(const void *pImage, SFaceItem *pFaces, int MaxNumFace) = 0;
};

class EEE : public TTT
{
public:
	int DDD(const void *pImage, SFaceItem *pFaces, int MaxNumFace);
private:
	CFaceDetector *mpFaceDetector;
	SFaceItems mFaces;
};

MULTIVIEWFACEDETECTORCLASSDLL_API TTT * GetInstance();
*/